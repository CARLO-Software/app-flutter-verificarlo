...
class SummaryTab extends ConsumerStatefulWidget {
  final int reportId;
  const SummaryTab({super.key, required this.reportId});
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () => _finalize(finalVerdict),
  ...
  void _finalize(String verdict) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('¿Finalizar con veredicto $verdict?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancelar')),
          FilledButton(
            onPressed: () {
              Navigator.pop(ctx);
              ref.read(inspectionProvider(widget.reportId).notifier).completeReport({
